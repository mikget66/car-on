import { DrivenType, PrismaClient } from "@prisma/client";
...
function mapDrivenType(type: string): DrivenType {
  if (type in drivenMapping) {
    return drivenMapping[type as keyof typeof drivenMapping] as DrivenType;
  }
...
async function main() {
  for (const rawCar of cars) {
    // Omit the auto-generated id from each record.
